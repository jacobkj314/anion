...
def filter_groups_by_questionlength(groups, question_length=15):
    passage_ids= groups.keys()
    some_long_ids=[]
    for passage_id in passage_ids:
        ...
        for edit_state in range(4):
            for each_q in annotation[edit_state]:
                print(each_q)
                if len(nltk.word_tokenize(each_q["sentence2"])) < question_length:
                    some_long_ids.append(passage_id)

    filtered_ids=[]
    for passage_id in some_long_ids:

        annotation = groups[passage_id]
        answers=[]
        for each_q in annotation[0]:
            answers.append(each_q["label"])

        if len(set(answers))!=1:
            filtered_ids.append(passage_id)
...
passage_ids = filter_groups_by_questionlength(groups, 15)
